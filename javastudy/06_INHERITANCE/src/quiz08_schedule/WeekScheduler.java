...
public class WeekScheduler {
		...
		for(int i = 0; i < week.length; i++) {
			if(dayName.equals(dayNames[i])) {
				if(week[i] == null) {	// 등록된 스케쥴이 없으면~~
					...
					String schedule = sc.nextLine();	// 스케쥴에 공백 입력 가능
					Day day = new Day();
			...
			if(dayName.equals(dayNames[i])) {
				if(week[i] == null) {	// 등록된 스케쥴이 없으면~~
					...
					if(yesNo.equalsIgnoreCase("y")) {
						...
						day.setSchedule(schedule);
						week[i] = day;
						System.out.println(dayName + "에 새 스케쥴이 등록되었습니다.");
					} else if(yesNo.equalsIgnoreCase("n")) {
						System.out.println("스케쥴 등록이 취소되었습니다.");
					} else {
						System.out.println("잘못 입력하여 처음으로 돌아갑니다.");
					}
				} else {	// 등록된 스케쥴이 있으면~~
					System.out.println(dayName + "의 스케쥴은 " + week[i].getSchedule() + "입니다.");
					...
					if(yesNo.equalsIgnoreCase("y")) {
						...
						System.out.println(dayName + "의 스케쥴이 변경되었습니다.");
					} else if(yesNo.equalsIgnoreCase("n")) {
	...
	public void deleteSchedule() {
		...
		for(int i = 0; i < week.length; i++) {
			if(dayName.equals(dayNames[i])) {
				if(week[i] == null) {	// 등록된 스케쥴이 없으면~~
					System.out.print(dayName + "요일은 스케쥴이 없습니다.");
				} else {	// 등록된 스케쥴이 있으면~~
	...
	public void manage() {
		...
		while(true) {
			...
			case 0 : System.out.println("스케쥴을 종료합니다."); return;
