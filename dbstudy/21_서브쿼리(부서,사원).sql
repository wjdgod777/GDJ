...
/*
    ...
    1. SUB QUERY
    2. 메인 쿼리(MAIN QUERY)에 포함하는 하위 커리(SUB QUERY)
...
-- 3. 부서번호가 1인 부서와 같은 지역에 있는 부서를 조회하기
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWID = 'AAAFCiAABAAALDBAAC';
...
-- 3) RANK() 함수
--    정렬 후 순위 매기는 함수
--    목록 가져오기에서는 부적절(동점자 처리때문에 가져오는 목록의 수가 매번 달라질 수 있음)
SELECT A.EMP_NO, A.NAME, A.SALARY
...
SELECT A.EMP_NO, A.NAME, A.HIRE_DATE
  FROM (SELECT RANK() OVER(ORDER BY HIRE_DATE ASC) AS 순위, EMP_NO, NAME, HIRE_DATE
          FROM EMPLOYEE) A
