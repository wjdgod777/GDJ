/*
    ...
    1. DUMMMY 칼럼에 'X' 값을 하나 가지고 있는 테이블
    2. 아무 의미 없는 테이블
...
/*
    ...
    2. TO_CHAR(데이터, '형식')   : 지정된 데이터(주로 숫자나 날짜)
...
SELECT
     ...
     , TO_CHAR(1.5, '9')        -- '1', 형식은 정수 1자리 표기(소수 이하 반올림)
     , TO_CHAR(0.123, '0.00')   -- '0.12', 소수 이하 2자리 표기(반올림)
     , TO_CHAR(0.129, '0.00')   -- '0.12', 소수 이하 2자리 표기(반올림)
  FROM
       DUAL;
...
-- 날짜 비교 연산은 TO_DATE함수를 사용하여 비교해야한다
...
SELECT DT1, DT2, DT3  -- 실행 O
  FROM SAMPLE
 WHERE TO_DATE(DT1, 'YY/MM/DD') = TO+DATE('22/08/26', 'YY/MM/DD');