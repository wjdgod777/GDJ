...
SELECT D.DEPARTMENT_NAME AS 부서명, COUNT(*) AS 사원수, TRUNC(AVG(SALARY)) AS 평균연봉
  FROM DEPARTMENTS D INNER JOIN EMPLOYEES E
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID  -- 조인조건
 GROUP BY D.DEPARTMENT_ID, D.DEPARTMENT_NAME
 ORDER BY 사원수;
 
SELECT D.DEPARTMENT_NAME AS 부서명, COUNT(*) AS 사원수, FLOOR(AVG(SALARY)) AS 평균연봉
  FROM DEPARTMENTS D, EMPLOYEES E
...
SELECT E.EMPLOYEE_ID, E.LAST_NAME, D.DEPARTMENT_NAME, L.CITY, C.COUNTRY_NAME
  FROM DEPARTMENTS D INNER JOIN EMPLOYEES E
    ...
    ON C.COUNTRY_ID = L.COUNTRY_ID;  -- 조인조건(조인조건은 NULL 제외된 상태로 처리)
