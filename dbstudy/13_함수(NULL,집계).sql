...
SELECT NVL(NAME, '아무개') AS NAME
     , NVL(KOR, 0) + NVL(ENG, 0) + NVL(MATH, 0) AS SUM
  FROM 
       SAMPLE;
...
SELECT
     ...
     , SUM(ENG)  -- 영어 합
     , SUM(MATH) -- 수학 합
--   , SUM(KOR, ENG, MATH)  인수(arguments)가 3개이므로 불가능(인수는 1개만)
